...
class RoutineGenerator(object):
    ...
    def get_routine(self, start_index_pos, target_index_pos):
        ...
        # merge routine
        if len(routine_to_start) > 0:
            routine_to_start.pop() # remove start point
        ...
        return self.simplify_routine(routine_to_start)
    ...
    def simplify_routine(self, routine_list):
        """ simplify the routine by combine nodes in a line """
        is_row_same = False
        is_col_same = False
        r_routine_list = [routine_list.pop(0)]
        while len(routine_list) > 0:
            next = routine_list.pop(0)
            if next[0] == r_routine_list[0][0]:
                if is_row_same:
                    r_routine_list.pop()
                else:
                    is_row_same = True
                    is_col_same = False
            elif next[1] == r_routine_list[0][1]:
                if is_col_same:
                    r_routine_list.pop()
                else:
                    is_row_same = False
                    is_col_same = True
            r_routine_list.append(next)
        return r_routine_list
